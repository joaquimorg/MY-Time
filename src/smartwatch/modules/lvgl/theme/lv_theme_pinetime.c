...
#define DARK_COLOR_SECONDARY    lv_color_hex(0x006600)

#define DARK_COLOR_SCR          lv_color_hex(0x000000)
#define DARK_COLOR_CARD         lv_color_hex(0x333333)
#define DARK_COLOR_TEXT         lv_palette_lighten(LV_PALETTE_GREY, 5)
#define DARK_COLOR_GREY         lv_color_hex(0x111111)
...
static void style_init(void)
{
    ...
    color_scr = DARK_COLOR_SCR;
    color_text = DARK_COLOR_TEXT;
    color_card = DARK_COLOR_CARD;
    color_grey = DARK_COLOR_GREY;
    ...
    lv_style_set_pad_right(&styles->scrollbar,  7);
    lv_style_set_pad_top(&styles->scrollbar,   7);
    ...
    lv_style_set_line_width(&styles->card,  1);

    style_init_reset(&styles->outline_primary);
    lv_style_set_outline_color(&styles->outline_primary, DARK_COLOR_PRIMARY);
    ...
    lv_style_set_outline_color(&styles->outline_secondary, DARK_COLOR_SECONDARY);
    ...
    lv_style_set_radius(&styles->btn,  16);
    ...
    lv_style_set_shadow_ofs_y(&styles->btn,  4);*/
    ...
    lv_style_set_pad_column(&styles->btn,  5);
    lv_style_set_pad_row(&styles->btn,  5);
    ...
    lv_style_set_pad_row(&styles->pad_gap,  10);
    lv_style_set_pad_column(&styles->pad_gap,  10);

    style_init_reset(&styles->line_space_large);
    lv_style_set_text_line_space(&styles->line_space_large,  20);
    ...
    lv_style_set_bg_color(&styles->bg_color_primary, DARK_COLOR_PRIMARY);
    ...
    lv_style_set_bg_color(&styles->bg_color_primary_muted, DARK_COLOR_PRIMARY);
    lv_style_set_text_color(&styles->bg_color_primary_muted, DARK_COLOR_PRIMARY);
    lv_style_set_bg_opa(&styles->bg_color_primary_muted, LV_OPA_20);

    style_init_reset(&styles->bg_color_secondary);
    lv_style_set_bg_color(&styles->bg_color_secondary, DARK_COLOR_SECONDARY);
    ...
    lv_style_set_bg_color(&styles->bg_color_secondary_muted, DARK_COLOR_SECONDARY);
    lv_style_set_text_color(&styles->bg_color_secondary_muted, DARK_COLOR_SECONDARY);
    ...
    lv_style_set_transform_width(&styles->grow,  3);
    lv_style_set_transform_height(&styles->grow,  3);
#endif

    style_init_reset(&styles->knob);
    lv_style_set_bg_color(&styles->knob, DARK_COLOR_PRIMARY);
    lv_style_set_bg_opa(&styles->knob, LV_OPA_COVER);
    lv_style_set_pad_all(&styles->knob,  6);
    ...
    lv_style_set_arc_width(&styles->arc_indic,  15);
    lv_style_set_arc_rounded(&styles->arc_indic, true);

    style_init_reset(&styles->arc_indic_primary);
    lv_style_set_arc_color(&styles->arc_indic_primary, DARK_COLOR_SECONDARY);
#endif
    ...
    lv_style_set_pad_all(&styles->cb_marker,  3);
    lv_style_set_border_width(&styles->cb_marker, BORDER_WIDTH);
    lv_style_set_border_color(&styles->cb_marker, DARK_COLOR_PRIMARY);
    ...
    lv_style_set_pad_all(&styles->switch_knob, -  4);
    ...
    lv_style_set_pad_column(&styles->chart_bg,  10);
    lv_style_set_line_color(&styles->chart_bg, color_grey);

    style_init_reset(&styles->chart_series);
    lv_style_set_line_width(&styles->chart_series,  3);
    lv_style_set_radius(&styles->chart_series,  3);
    lv_style_set_size(&styles->chart_series,  8);
    lv_style_set_pad_column(&styles->chart_series,  2);

    style_init_reset(&styles->chart_indic);
    lv_style_set_radius(&styles->chart_indic,LV_RADIUS_CIRCLE);
    lv_style_set_size(&styles->chart_indic,  8);
    lv_style_set_bg_color(&styles->chart_indic, DARK_COLOR_PRIMARY);
    lv_style_set_bg_opa(&styles->chart_indic, LV_OPA_COVER);

    style_init_reset(&styles->chart_ticks);
    lv_style_set_line_width(&styles->chart_ticks,  1);
    lv_style_set_line_color(&styles->chart_ticks, color_text);
    lv_style_set_pad_all(&styles->chart_ticks,  2);
    lv_style_set_text_color(&styles->chart_ticks, lv_palette_main(LV_PALETTE_GREY));
#endif

#if LV_USE_METER
    style_init_reset(&styles->meter_marker);
    lv_style_set_line_width(&styles->meter_marker,  5);
    lv_style_set_line_color(&styles->meter_marker, color_text);
    lv_style_set_size(&styles->meter_marker,  20);
    lv_style_set_pad_left(&styles->meter_marker,  15);
    ...
    lv_style_set_size(&styles->meter_indic,  15);
#endif
    ...
    lv_style_set_pad_left(&styles->ta_cursor,  1);
    lv_style_set_border_side(&styles->ta_cursor, LV_BORDER_SIDE_LEFT);
    lv_style_set_anim_time(&styles->ta_cursor, 400);

    style_init_reset(&styles->ta_placeholder);
    lv_style_set_text_color(&styles->ta_placeholder, (lv_palette_darken(LV_PALETTE_GREY, 2));
#endif
    ...
    lv_style_set_border_width(&styles->calendar_day,  1);
    ...
    lv_style_set_arc_width(&styles->colorwheel_main,  10);
#endif

#if LV_USE_MSGBOX
    /*To add space for for the button shadow*/
    style_init_reset(&styles->msgbox_btn_bg);
    lv_style_set_pad_all(&styles->msgbox_btn_bg,  4);
    ...
    lv_style_set_radius(&styles->keyboard_btn_bg, RADIUS_DEFAULT / 2);
#endif

#if LV_USE_TABVIEW
    style_init_reset(&styles->tab_btn);
    lv_style_set_border_color(&styles->tab_btn, DARK_COLOR_PRIMARY);
    ...
    lv_style_set_border_width(&styles->list_btn,  1);
    ...
    lv_style_set_shadow_width(&styles->led,  15);
    lv_style_set_shadow_color(&styles->led, lv_color_white());
    lv_style_set_shadow_spread(&styles->led,  5);
#endif    

}

lv_theme_t * lv_theme_pinetime_init(void) {
